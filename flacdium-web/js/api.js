...
import { isFlacdiumMode, flacdiumRoute, flacdiumLibraryRaw } from './flacdium-api.js';
...
export class LosslessAPI {
    ...
    async fetchWithRetry(relativePath, options = {}) {
        ...
        // Flacdium mode: serve every data request from the shared-library JSON API
        // instead of TIDAL. Returns a Response-like object with .json().
        if (isFlacdiumMode()) {
            return await flacdiumRoute(relativePath);
        }
    ...
    // Full Flacdium library, paginated. Used by the home/landing browse view.
    async getLibraryTracks(page = 1, sort = 'newest') {
        if (!isFlacdiumMode()) return { tracks: [], total: 0, hasNext: false, page, totalPages: 1 };
        const raw = await flacdiumLibraryRaw(page, sort);
        return {
            tracks: raw.items.map((t) => this.prepareTrack(t)),
            total: raw.total,
            hasNext: raw.hasNext,
            page: raw.page,
