...
export class UIRenderer {
    ...
    async renderHomePage() {
        ...
        try {
            ...
            // Flacdium is a small shared library, so the landing page browses the whole
            // catalog (paginated) instead of TIDAL-style recommendations.
            const page = document.getElementById('page-home');
            if (!page) return;
            page.innerHTML = `
                <div class="content-section" style="padding-top: 0.5rem">
                    <div class="section-header-row" style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:1rem">
                        <h2 class="section-title" style="margin:0">Library</h2>
                        <div style="display:flex;align-items:center;gap:12px">
                            <span id="library-count" style="color:var(--muted-foreground);font-size:0.85rem"></span>
                            <select id="library-sort" class="btn-secondary" style="padding:4px 8px">
                    ...
                    <div class="track-list" id="library-track-list"></div>
                    <div style="text-align:center;margin:1.5rem 0">
                        <button class="btn-secondary" id="library-load-more" style="display:none">Load more</button>
                    </div>
                </div>`;

            const sortEl = page.querySelector('#library-sort');
            this._librarySort = this._librarySort || 'newest';
            if (sortEl) {
                sortEl.value = this._librarySort;
                sortEl.onchange = () => {
                    this._librarySort = sortEl.value;
                    this._loadLibraryTracks(true);
                };
            }
            await this._loadLibraryTracks(true);
        } catch (e) {
            console.error('renderHomePage (library browse) failed:', e);
        } finally {
            this.renderLock = false;
        }
    }

    async _loadLibraryTracks(reset) {
        const list = document.getElementById('library-track-list');
        const moreBtn = document.getElementById('library-load-more');
        const countEl = document.getElementById('library-count');
        if (!list) return;
        if (reset) {
            this._libraryPage = 1;
            this._libraryLoaded = 0;
            list.innerHTML = this.createSkeletonTracks(12, true);
        }
        try {
            const res = await this.api.getLibraryTracks(this._libraryPage, this._librarySort || 'newest');
            if (reset) list.innerHTML = '';
            if (res.tracks.length) {
                await this.renderListWithTracks(list, res.tracks, true, true, false, true);
            } else if (reset) {
                list.innerHTML = createPlaceholder('Library is empty. Upload FLAC on Flacdium to get started.');
            }
            this._libraryLoaded = (this._libraryLoaded || 0) + res.tracks.length;
            if (countEl) countEl.textContent = `${this._libraryLoaded} / ${res.total} tracks`;
            if (moreBtn) {
                if (res.hasNext) {
                    moreBtn.style.display = '';
                    moreBtn.onclick = () => {
                        this._libraryPage = (this._libraryPage || 1) + 1;
                        this._loadLibraryTracks(false);
                    };
                } else {
            ...
            if (reset) list.innerHTML = createPlaceholder('Failed to load library.');
        }
    }
