...
import { flacdiumStreamUrl, hasFlacdiumToken } from './flacdium-api.js';
...
export class Player {
    ...
    async playTrackFromQueue(startTime = 0, recursiveCount = 0, isRetry = false) {
        ...
        try {
            ...
            // Flacdium streaming requires a signed-in account. Without a token every /preview
            // request 403s, which the error handler would treat as a bad file and skip — looping
            // through the whole queue. Stop here and prompt sign-in instead of skipping.
            if (track.isFlacdium && !hasFlacdiumToken()) {
                console.warn('Flacdium: sign in required to stream.');
                try {
                    activeElement.pause();
                } catch {
                    /* ignore */
                ...
                window.dispatchEvent(new CustomEvent('flacdium:need-login'));
                return;
            }
            ...
            if (isPodcast) {
                ...
                // Flacdium tracks: rebuild the /preview URL with the CURRENT auth token so a
                // track browsed before signing in still streams once the user logs in.
                if (track.isFlacdium) {
                    streamUrl = flacdiumStreamUrl(track.id);
                }
