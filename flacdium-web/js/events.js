...
export function initializePlayerEvents(player, audioPlayer, scrobbler, ui) {
    ...
    if (homeStartRadioBtn) {
        const cleanupFn = cleanupManager.addEventListener(
            homeStartRadioBtn,
            'click',
            async () => {
        ...
        cleanupManager.register('player:home-start-radio', cleanupFn);
    }
    ...
    const setupMediaListeners = (element) => {
        ...
        element.addEventListener('error', (e) => {
            ...
            if (player.currentTrack && error && error.code !== 1) {
                ...
                cleanupManager.setTimeout(() => player.playNext(), 1000);
            }
        });
