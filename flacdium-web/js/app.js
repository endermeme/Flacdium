...
import { initLibrarySync } from './flacdium-sync.js';
...
import { getCleanupManager } from './cleanup-manager.js';
...
if (typeof window !== 'undefined') {
    ...
    // analytics — external tracking disabled for Flacdium (self-hosted). Keep the
    // window.plausible no-op stub so existing plausible(...) calls stay harmless.
    window.plausible =
        window.plausible ||
        function () {
...
document.addEventListener('DOMContentLoaded', async () => {
    // Bind the auth-wall login gate first so it is interactive immediately, even while
    // the rest of the app boots behind it.
    authManager.setupGate();

    // Pull the user's synced library (favorites/playlists/history) from the server into
    // IndexedDB before the UI reads it, then keep pushing changes back. No-op if not signed in.
    await initLibrarySync();

    await modernSettings.waitPending();

    // Initialize cleanup manager and memory monitor
    const cleanupManager = getCleanupManager();
    const memoryMonitor = getMemoryMonitor({
        enabled: import.meta.env.DEV, // Enable in development mode
        checkInterval: 30000, // Check every 30 seconds
        memoryGrowthThreshold: 10 * 1024 * 1024, // 10MB threshold
    });

    // Start memory monitoring in development
    if (import.meta.env.DEV) {
        memoryMonitor.start();
        console.log('📊 Memory monitoring started - use window.getMemoryMonitor().logStats() to check stats');
    }
    ...
    // Also cleanup on visibility change (when user closes tab)
    ...
    new ThemeStore();
    // Flacdium mode serves all data from its own API (see flacdium-api.js), so the TIDAL
    // HiFiClient is never queried. Skip its init entirely to avoid the TIDAL OAuth network
    // round-trip on boot (it was a measurable chunk of startup latency).
    if (window.FLACDIUM_MODE === false) {
        try {
            ...
            console.warn('HiFiClient (TIDAL) init failed:', err);
        }
    }
    ...
    if (isIos || isSafari) {
        ...
        if (isIos) {
            // The hi-res-download-warning element was removed with the quality settings; guard it.
            const warn = document.querySelector('#hi-res-download-warning');
    ...
    // Tracker (Artistgrid "unreleased") and contributors (samidy cloud) are TIDAL/cloud
    // features removed from Flacdium — skipping them drops several boot network calls,
    // including a blocking awaited fetch that waited on a failing cross-origin request.
    ...
    if (headerAccountBtn && headerAccountDropdown) {
        ...
        async function updateAccountDropdown() {
            ...
            if (!user) {
                // Behind the auth wall this is rarely seen; offer to reopen the login gate.
                headerAccountDropdown.innerHTML =
                    '<button class="btn-secondary" id="header-signin-open" style="width:100%">Sign In</button>';
                document.getElementById('header-signin-open').onclick = () => {
                    document.getElementById('flacdium-auth-gate')?.classList.remove('hidden');
                    headerAccountDropdown.classList.remove('active');
                };
            } else {
                headerAccountDropdown.innerHTML = `
                    <div style="padding:0.5rem 0.75rem;color:var(--muted-foreground);font-size:0.85rem">Signed in as <b style="color:var(--foreground)">${escapeHtml(user.username)}</b></div>
                    <hr style="border:none;border-top:1px solid var(--border);margin:0.25rem 0">
                    <button class="btn-secondary danger" id="header-sign-out" style="width:100%">Sign Out</button>
                `;
        ...
        authManager.onAuthStateChanged(() => {
            // Flacdium has no avatars; always show the default account icon.
